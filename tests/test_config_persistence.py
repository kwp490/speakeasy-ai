...
class TestDevPanelValidation:
    ...
    def test_validate_clamps_undersized_height(self):
        ...
        s.validate()
        assert s.dev_panel_height == 720

    @pytest.mark.parametrize("tab", ["settings", "realtime", "logs", "pro"])
    def test_validate_accepts_all_four_valid_tab_keys(self, tab):
        s = Settings(dev_panel_active_tab=tab)
        s.validate()
        assert s.dev_panel_active_tab == tab

    def test_validate_accepts_valid_dimensions(self):
        s = Settings(dev_panel_width=600, dev_panel_height=800)
        s.validate()
